...
#       ultracode 關閉的連帶（settings.json 不能帶註解，記這裡）：持久 effort 設定（effortLevel／modelSettings）生效——ultracode=true
#       時整張 modelSettings 被丟棄，effort 逐鍵理由記在 tests/repo-integrity.sh §9；哪些世代有 model-default hold 會蓋掉持久設定，以
#       https://code.claude.com/docs/en/model-config 為準（2026-09-05 查：Opus 5／Fable 5.1 無 hold）；concurrent subagent 上限回 20（binary 確認），dev steer 只影響 model 層編排決定、恢復不了那個豁免。
