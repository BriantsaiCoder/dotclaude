...
# 兩個放行狀態都用「STATE=X 後面接空白」精確比對，不用 `STATE=PASS*` 這種前綴 glob。
# 前綴 glob 會自動吃掉未來任何以 PASS 開頭的新狀態——gate 那邊新增一個 STATE=PASS_ANYTHING，
# 這裡就無聲放行了，沒有人需要同意。降級路徑正是這種形狀，所以它必須是**明寫的一條**，
# 不是 glob 順便涵蓋到的。
#
